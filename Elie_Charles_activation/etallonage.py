...
for peak_indexes in file_peaks:
    ...
    print(fit_mu + peak_indexes[0],fit_ampl)
...
plt.errorbar(means,peak_energies, xerr=np.array(stdevs), fmt = '.',c = "red", alpha = 0.75)
text = ["supérieur", "moyen", "inférieur"]
droites = []
for i in range(3):
    popt, pcov = curve_fit(line, means + (1 - i)*np.array(stdevs), peak_energies)
    perr = np.sqrt(np.diag(pcov))
    droites.append(popt)
    if i == 1:
